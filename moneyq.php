...
error_reporting(0);
/** COPYRIGHT
@@ CREATOR : Vj Rusmayana
@@ DDMMYY  : 20 Desember 2018
**/
...
function reff($reff, $pass)
{
    ...
    $ch             = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'http://moneymate.cutelove.in/webservices/signup.php?device_id=' . $device . '&name=' . $getName . '%20' . $getName2 . '&phone=' . $no . '&email=' . $getName . '' . $rand . '@gmail.com&password=' . $pass . '' . $rand . '&refferal_id=' . $reff . '');
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    $result = curl_exec($ch);
    return $result;
}
print "MoneyQ Suntik Reff\n";
print "Creator : Vj Rusmayana\n";
echo "Refferal : ";
